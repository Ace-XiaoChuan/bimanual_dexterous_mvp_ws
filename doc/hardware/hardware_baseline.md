...
| `connector_plug` | 航空插头本体坐标系 | 后续插接阶段定义 |
| `connector_socket` | 航空插座或目标接口坐标系 | 后续插接阶段定义 |
...
* 航空插头插接阶段还需要定义插头键位方向、插座坐标系、插接轴线和末端接触坐标系。

## 6. 仿真与真实硬件接入路线

硬件能力应按层次逐步接入，不应从完整 Pick-and-Place 或航空插头插接直接上真机。
...
| 任务集成 | 能把硬件能力接回任务系统 | 先单臂单手，再考虑双臂或航空插头插接 |
